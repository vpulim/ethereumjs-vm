...
module.exports = {
  ...
  SSTORE: function (key, val, runState, cb) {
    ...
    var stateManager = runState.stateManager
    var originalState = runState.originalState
    ...
    async.parallel([
      originalState.getContractStorage.bind(originalState, address, key),
      stateManager.getContractStorage.bind(stateManager, address, key)
    ], function (err, found) {
      if (err) return cb(err)
        ...
        if (current.equals(value)) {
          ...
          if (original.equals(current)) {
            ...
            if (original.length === 0) {
              ...
              subGas(runState, new BN(runState._common.param('gasPrices', 'netSstoreInitGas')))
            } else if (value.length === 0) {
              // If new value is 0, add 15000 gas to refund counter.
              runState.gasRefund = runState.gasRefund.addn(runState._common.param('gasPrices', 'netSstoreClearRefund'))
            } else {
              // Otherwise, 5000 gas is deducted.
              subGas(runState, new BN(runState._common.param('gasPrices', 'netSstoreCleanGas')))
            }
          } else {
            // If original value does not equal current value (this storage slot is dirty), 200 gas is deducted. Apply both of the following clauses.
            subGas(runState, new BN(runState._common.param('gasPrices', 'netSstoreDirtyGas')))
            if (original.length !== 0) {
